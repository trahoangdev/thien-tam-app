...
export const uploadBook = async (req: Request, res: Response) => {
  ...
  const tempCoverPath =
    req.files && (req.files as any).cover
  ...
  try {
    ...
    const coverFile = (req.files as any).cover
      ? (req.files as any).cover[0]
      : null;
    ...
    if (coverFile) {
      ...
      coverUploadResult = await cloudinaryService.uploadCoverImage(
        tempCoverPath!,
        {
  ...
  try {
    ...
    if (category) {
      ...
      if (mongoose.Types.ObjectId.isValid(category)) {
        ...
        // If not ObjectId, lookup by name
        const categoryDoc = await BookCategory.findOne({
          $or: [{ name: category }, { nameEn: category }],
        });
        if (categoryDoc) {
          filter.category = categoryDoc._id;
        }
        // If category not found, filter will return empty (no match)
      }
    }
    ...
    // Execute query - handle books with invalid category references
    let books;
    try {
      ...
      // If populate fails due to invalid ObjectId in data, fetch without populate
      console.warn(
        "[BOOK] Populate error (invalid category IDs in DB):",
